...
if __name__ == '__main__':
    ...
    arm_runtime_helpers = (
        'THUNK_TRACE("__adddf3",',
        'THUNK_TRACE("__addsf3",',
        'THUNK_TRACE("__aeabi_',
    )
    in_arm_runtime_block = False
    for l in output:
        is_arm_runtime = l.startswith(arm_runtime_helpers)
        if is_arm_runtime and not in_arm_runtime_block:
            impl_tab.write("#if defined(__arm__)\n")
            in_arm_runtime_block = True
        elif not is_arm_runtime and in_arm_runtime_block:
            impl_tab.write("#endif /* __arm__ */\n")
            in_arm_runtime_block = False
        impl_tab.write("{}\n".format(l))
    if in_arm_runtime_block:
        impl_tab.write("#endif /* __arm__ */\n")
